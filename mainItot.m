...
% 
% recuperationAlarmesAlternateurs;
...
for indice = 1:nbAlternateurs
   ...
   %% Gestion Alarmes 
    ...
    %% Corrélation Puissance active//Puissance réactive
    sigma_it = corrcoef (MXLocal,MWLocal);
    sigma_it = sigma_it(1,2);
    
    sigma = [sigma;sigma_it];
...
%       vecteur_instances = instance*ones(length(MXcap_hiv),1);
%       vecteur_groupes   = numeroAlternateur*ones(length(MXcap_hiv),1);
%       tablelimite_it    = [vecteur_instances vecteur_groupes MXcap_hiv MWcap_hiv];
%       
%       tablelimite = vertcat (tablelimite,tablelimite_it)   ;
%      
%  end   
...
%     saveas(fig_espace_guassien,"Figures/FiguresOPEspaceGaussien/20102018/Courbe de conception " + nomCentraletGroupe +".png")
%      
...
%     %% Tracé de l'espace opérationnelle
    ...
    saveas(fig_carac,'Figures/FiguresOP_Carac/'+nomCentraletGroupe+'.png');
    saveas(fig_sanscarac,'Figures/FiguresOP_sansCarac/'+nomCentraletGroupe+'.png');
...
tablenom.Properties.VariableNames = ["Nom"];

tableResultats = array2table(resultats_finaux(:,:));
tableResultats.Properties.VariableNames = ["Num_Centrale","Num_Alternateur","Beta_ete","Beta_ete_active","Beta_ete_reactive","Beta_hiver",...
                                            "Beta_hiver_active","Beta_hiver_reactive","Pourcentage_motorisation","PDegradationEte","PDegradationHiver"...
                                            "tauDepassement"];

                   
tableResultats = horzcat(tablenom,tableResultats);
%resultats = rmmissing(resultats);
 writetable(tableResultats,'Résultats/BETA_PARC_HYDRO.csv','Delimiter',';');
